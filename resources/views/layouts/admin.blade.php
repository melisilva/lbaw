...
<html lang="{{ app()->getLocale() }}">
  <head>
    ...
    <title>{{ config('Noodle', 'A melhor escolha da tua vida académica! (lbaw2152)') }}</title>
  ...
  <body style="height: auto;width: auto;max-width: auto;">
      ...
      @elseif(Auth::check())
      <header>
      @include('headers.navBarAuth')
      </header>
      @include('pages.forbidden')
      @else
      <header>
      @include('headers.navBarNotAuth')
      </header>
      @include('pages.forbidden')
      @endif
